...
async def send_welcome_dm(user_id: int, bot: Bot, full_name: str):
    """Function to send the full welcome package (Video + APK) sequentially for maximum reliability"""
    welcome_caption = (
    ...
    # 1. Send Video with Welcome Caption First
    try:
        if FILE_ID_CACHE["video"]:
            ...
            logger.warning(f"Video file NOT FOUND at: {os.path.abspath(VIDEO_PATH)}")
